...
public class PendingAppointmentAdapter extends RecyclerView.Adapter<PendingAppointmentAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        // Use public fields directly to fix compilation errors
        ...
        holder.cancel.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                new AlertDialog.Builder(activity)
                        ...
                        .setMessage("Are you sure you want to cancel the appointment request with Dr. " + request.Name + " for " + request.DateAndTime + "?")
                        .setPositiveButton("Yes", new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialog, int which) {
                                ...
                                FirebaseDatabase.getInstance().getReference().child("PendingDocAppointments").child(request.DocID).child(request.DoctorAppointKey).removeValue().addOnCompleteListener(new OnCompleteListener<Void>() {
                                    @Override
                                    public void onComplete(@NonNull Task<Void> task) {
                                        if (task.isSuccessful()) {
                                            FirebaseDatabase.getInstance().getReference().child("PendingPatientAppointments").child(FirebaseAuth.getInstance().getCurrentUser().getUid()).child(request.PatientAppointKey).removeValue().addOnCompleteListener(new OnCompleteListener<Void>() {
                                                @Override
                                                public void onComplete(@NonNull Task<Void> task) {
                                                    ...
                                                    if (task.isSuccessful()) {
                                                        ...
                                                        ReusableFunctionsAndObjects.showMessageAlert(activity, "Error", "Could not cancel request.", "OK", (byte) 0);
                                                    }
                                                }
                                            });
                                        } else {
